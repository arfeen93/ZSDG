...
def main(args):
	...
	for best_model_name in os.listdir(path_cp):
		...
		if os.path.isfile(best_model_file):
			...
			per_class_acc, acc_overall = compute_zsl_acc(te_loader, model, dict_clss_te, seen_classes, epoch, args)

			outstr = "\n".join("{!r}: {!r},".format(k, v) for k, v in per_class_acc.items())
			outstr += '\n\nOverall:'+'{0:.4f}'.format(acc_overall)
			
			print(outstr)
			result_file = open(os.path.join(path_log_save, 'seen_cls_'+best_model_name[:-len('.pth')]+'.txt'), 'w')
